          ...
          <div class="col-md-7">
            <div class="box">
              <div class="box-header with-border">
                <h3 class="box-title"><i class="fa fa-truck"></i> Competencias</h3>

                <button type="button" class="btn btn-xs btn-default pull-right" @click="fillCourses()">
                ...
                <table class="table table-striped">
                  ...
                  <tbody>
                    <tr v-for="(val, index) in filtered">
                      ...
                      <td>
                        <button type="button" class="btn btn-success btn-sm" v-show="val.editabled" @click="updateCourses(val)">
                          <i class="fa fa-floppy-o"></i>
                        </button>
                        <button type="button" class="btn btn-warning btn-sm" @click="turnEditabled(val)" :disabled="val.editabled">
                          <i class="fa fa-pencil"></i>
                        </button>
                        <button type="button" class="btn btn-danger btn-sm" @click="deleteCourses(val, index)" :disabled="val.editabled">
          ...
          <div class="col-md-5">
            <div class="box">
              <div class="box-header with-title">
                <h3 class="box-title"><i class="fa fa-plus-circle"></i> Agregar Competencias</h3>
              </div>
              <div class="box-content">
                <div class="container-fluid">
                  <div class="row">
                    <form class="form form-horizontal">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-6">
                          ...
                          <button type="button" class="btn btn-primary" @click="saveCourses">
                            Agregar
  ...
  methods: {
    ...
    fillCourses() {
      if ((this.permissions.find(cb => {
        ...
        this.turnLoad()
        axios.get(this.url+'/api/courses').then(res => {
          this.courses = []
          res.data.forEach(dt => {
            this.courses.push({
              id: dt.id,
    ...
    saveCourses() {
      if ((this.permissions.find(cb => {
        ...
        if (this.input.name.trim() != '' && this.input.code.trim() != '') {
          this.turnLoad()
          axios.post(this.url+'/api/courses',
            {name: this.input.name, code: this.input.code.toUpperCase()})
          .then(res => {
            switch (res.data) {
              case 'duplicate':
    ...
    deleteCourses(val, index) {
      if (this.permissions.find(cb => {
        return cb.name === 'delete courses'
      })) {
        if (confirm('Desea borrar este registro')) {
          this.turnLoad()
          axios.delete(this.url+'/api/courses/'+val.id).then(res => {
            switch (res.data) {
              case true:
              ...
              case 'unique':
                alert(
                  'Otros campos dependen de este, primero borre las dependencias'
                )
                break;
            }
          }).catch(er => {
